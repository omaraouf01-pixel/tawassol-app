...
export default function GroupSettings({ isOpen, onClose, groupData, setToast }) {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            setToast("Node metadata synchronized successfully.");
            onClose();
        } catch (error) {
            console.error(error);
            setToast("Failed to update protocol.");
        } finally {
            setIsSaving(false);
        }
    };
            ...
            {isOpen && (
                <>
                    ...
                    <motion.aside
                        ...
                        className="fixed top-0 right-0 h-screen w-full sm:w-[420px] bg-[#050505] border-l border-white/10 z-[170] flex flex-col shadow-2xl"
                    >
                        <div className="p-8 border-b border-white/5 flex items-center justify-between bg-[#0A0A0B]">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-lg font-serif font-black italic text-white leading-none">Node Configuration</h3>
                                    <p className="text-[7px] font-black uppercase text-slate-600 tracking-[0.3em] mt-2">Overseer Level Access</p>
                                </div>
                            </div>
                            <button onClick={onClose} className="p-2 bg-white/5 text-slate-500 hover:text-white rounded-xl border-none cursor-pointer transition-colors"><X size={18} /></button>
                        </div>

                        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                            {/* Node Name */}
                            <div className="space-y-2">
                                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-2"><Edit3 size={10} /> Node Identity</label>
                                <input
                                    value={formData.name}
                            ...
                            {/* Subject */}
                            <div className="space-y-2">
                                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Academic Subject</label>
                                <input
                            ...
                            {/* Description */}
                            <div className="space-y-2">
                                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-2"><Info size={10} /> Node Objective</label>
                                <textarea
                            ...
                            {/* Protocol Rules */}
                            <div className="space-y-2">
                                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-2"><Lock size={10} /> Protocol Rules</label>
                                <textarea
                                    value={formData.rules}
                                    onChange={(e) => setFormData({ ...formData, rules: e.target.value })}
                                    rows={4}
                                    placeholder="Set the laws of this sanctuary..."
